...
import { Plus, CalendarClock, Loader2, AlertCircle, CheckCircle2, Users } from 'lucide-react';
      ...
      <header className="border-b border-border/60 bg-background/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-lg tracking-tight">MeetMint</h1>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
          ...
          {loadingParticipants ? (
            ...
            <div className="text-center py-12 space-y-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setAddDialogOpen(true)}>
                <Plus className="h-3 w-3 mr-1" /> Add your first participant
              </Button>
            </div>
          ) : (
        ...
        <section className="space-y-6">
          ...
          <div className="rounded-xl border border-border/60 bg-card p-5 space-y-4">
            ...
            <Separator />
            <SlotSearchForm onResults={setSearchResult} />
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm ${
                ...
                {searchResult.noFullOverlap ? (
                  <>
                    ...
                    <span>
                      No full-group overlap found. Showing best partial matches
                      ({searchResult.totalCandidatesEvaluated} windows evaluated,
                       strategy: <strong>{searchResult.strategy}</strong>).
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span>
                      Full overlap found! {searchResult.slots.length} slot
                      {searchResult.slots.length !== 1 ? 's' : ''} available.
                    </span>
                  </>
                )}
              </div>
              ...
              {searchResult.slots.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground text-sm space-y-1">
                  <p className="font-medium">No slots found</p>
                  <p>Try widening the date range or reducing the duration.</p>
                </div>
              ) : (
                <ScrollArea className="h-[calc(100vh-28rem)]">
                  <div className="space-y-3 pr-3">
                    {searchResult.slots.map((slot, i) => (
                      <SlotResultCard key={slot.startUtc} slot={slot} rank={i + 1} />
                    ))}
                  </div>
                </ScrollArea>
              )}
